...
class Block_Controller(object):
    ...
    def dropDown(self, board, Shape_class, direction, x):
        ...
        for _x, _y in coordArray:
            _yy = 0
            while _yy + _y < self.board_data_height and (_yy + _y
< 0 or board[(_y + _yy) * self.board_data_width + _x] == self.ShapeNone_index):
                _yy += 1
            _yy -= 1
            if _yy < dy:
                dy = _yy
    ...
    def calcEvaluationValueSample(self, board):
        ...
        score = score - nHoles * my_param.c_nHoles           #1.0
          # try not to make hole
        score = score - nIsolatedBlocks * my_param.c_nIsolatedBlocks #1.0   # try not to make isolated block
        score = score - absDy * my_param.c_absDy             #1.0
          # try to put block smoothly
        #score = score - maxDy * my_param.c_maxDy         #0.3       # maxDy
        #score = score - maxHeight * my_param.c_maxHeight #5         # maxHeight
        #score = score - stdY * my_param.c_stdY           #1.0       # statistical data
        #score = score - stdDY * my_param.c_stdDY         #0.01      # statistical data
